...
function nextCount(){
    ...
    // alert("old:"+totChangX +" "+ totChangY + " " + oldRotation + " New:" + newX + " " +newY + " " + newRotation+ " " + changRot);
}
...
paper.view.onFrame = function(event) { 
    ...
    // the frame variables outline how much it should move in which direction, 
    // this allows the turtle to take the shortest route
    var frameX;
    var frameY;
    ...
    if (totChangX > 0) {
	if (inChangX < totChangX){
		inChangX += (frameX * moveSpeed);
		oldX += (frameX * moveSpeed);
		if(turtleShow==1){
	    		tail.translate((frameX * moveSpeed),0);
	   		 circle1.translate((frameX * moveSpeed),0);
			 circle2.translate((frameX * moveSpeed),0);
			 circle3.translate((frameX * moveSpeed),0);
			 circle4.translate((frameX * moveSpeed),0);
			 circle5.translate((frameX * moveSpeed),0);
			 circle6.translate((frameX * moveSpeed),0);
		}
	}
	else if (inChangX > totChangX){
		inChangX -= (totChangX % moveSpeed);
		oldX -= (totChangX % moveSpeed);
	}
    }
    ...
    if (totChangY > 0) {
	if (inChangY < totChangY){
		inChangY += (frameY * moveSpeed);
		oldY += (frameY * moveSpeed);
		if(turtleShow==1){
    		    tail.translate(0,(frameY * moveSpeed));
		    circle1.translate(0,(frameY * moveSpeed));
		    circle2.translate(0,(frameY * moveSpeed));
		    circle3.translate(0,(frameY * moveSpeed));
		    circle4.translate(0,(frameY * moveSpeed));
		    circle5.translate(0,(frameY * moveSpeed));
		    circle6.translate(0,(frameY * moveSpeed));

		}
	}
	else if (inChangY > totChangY) {
		inChangY -= (totChangY % moveSpeed);
		oldY -= (totChangY % moveSpeed);
	}
    }
...
*/
    ...
    // to create the line
    if ((inChangX != totChangX || inChangY != totChangY || changRot != 0)){
